...
namespace Fusion.StatsInternal {
  ...
  public static class FusionStatsUtilities {
    ...
    public static List<string> CachedTelemetryNames {
      get {
        if (_cachedGraphVisualizationNames == null) {
          _cachedGraphVisualizationNames = new List<string>(System.Enum.GetNames(typeof(FusionGraphVisualization)));
        }
    ...
    static Texture2D MeterTexture {
      get {
        if (_meterTexture == null) {
          _meterTexture = new Texture2D(16, 16);
          for (int i = 0; i < 16; ++i) {
            for (int y = 0; y < 16; ++y) {
              _meterTexture.SetPixel(i, y, i == 15 ? new Color(0.5f, 0.5f, 0.5f, 0.0f) : new Color(1f, 1f, 1f, 1f));
            }
          }
          _meterTexture.Apply();
        }
    ...
    public static bool TryFindActiveRunner(FusionStats fusionStats, out NetworkRunner runner, SimulationModes? mode = null) {
      ...
      //  scene.FindObjectsOfTypeInOrder(_reusableList, false);
      //  foreach (var r in _reusableList) {
      //    Debug.LogWarning($"Try Find {gameObject.name} {r.IsRunning} {r.gameObject.scene.name }:{ gameObject.scene.name} {r.IsRunning}]");

      //    if (r.IsRunning && (r.gameObject.scene == scene || r.SimulationUnityScene == scene)) {
      //      runner = r;
      //      return true;
      //    }
      //  }
    ...
    //public static RectTransform CreateRectTransform(string name, Transform parent, bool expand = false) {
    //  var go = new GameObject(name);
    //  var rt = go.AddComponent<RectTransform>();
    //  rt.SetParent(parent);
    //  rt.localScale = new Vector3(1, 1, 1);
    ...
    public static UI.Dropdown CreateDropdown(this RectTransform rt, float padding, Color fontColor) {
      ...
      var itemLablRT = itemRT.CreateRectTransform("Item Label", true)
        .SetOffsets(20 + padding, -padding, padding, -padding);
